...
class Polygon{
    public:
        Polygon(const std::vector<float>& data, const std::vector<unsigned int>& indices, const std::vector<Property>& properties=std::vector<Property>(), bool position2D = true, bool texture=true, bool color=false);
        Polygon();
        Polygon(vec2 scale);
        ...
        void setVertices(const std::vector<float>& data, const std::vector<unsigned int>& indices);
